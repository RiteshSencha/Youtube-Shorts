...
def generate_ai_images(sentences, topic_title, out_dir, count=4):
    ...
    # Build prompts tightly based on actual topic
    base = f"{topic_title}, science photography, dramatic lighting, dark background, 4k photorealistic"
    prompts = [
        f"extreme macro closeup of {topic_title}, cinematic, dark moody",
        f"cinematic wide shot of {topic_title}, documentary style, atmospheric",
        f"detailed scientific visualization of {topic_title}, dramatic lighting",
        f"nature documentary shot of {topic_title}, high contrast, vivid",
    ]
    ...
    for i in range(num_images):
        ...
        try:
            ...
            # Validate it's a real image (must be > 10KB)
            if len(resp.content) < 10000:
                raise ValueError(f"Image too small: {len(resp.content)} bytes")
            with open(img_path, "wb") as f:
                f.write(resp.content)
            images.append(img_path)
            logger.info(f"AI image {i+1} saved ({len(resp.content)//1024}KB)")
...
def _create_gradient(output_path, index=0):
    # Save as PNG for better FFmpeg compatibility
    ...
    if result.returncode == 0:
        return png_path
    # Ultra fallback — write a plain black PNG via Python
    try:
...
def assemble_video(topic, audio_path, narration, output_path="output/final.mp4"):
    ...
    # Use actual topic for relevant image prompts
    topic_hint = topic.get("pexels_search", topic.get("title", "science"))
