...
KEN_BURNS_EFFECTS = [
    "zoompan=z='min(zoom+0.0008,1.3)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d={d}:s=1080x1920:fps=30",
    "zoompan=z='if(lte(zoom,1.0),1.3,max(1.001,zoom-0.0008))':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d={d}:s=1080x1920:fps=30",
    "zoompan=z='min(zoom+0.0006,1.25)':x='iw/2-(iw/zoom/2)+{drift}*(on/{d})':y='ih/2-(ih/zoom/2)':d={d}:s=1080x1920:fps=30",
    "zoompan=z='1.2':x='iw/2-(iw/zoom/2)+50*(on/{d})':y='ih/2-(ih/zoom/2)-30*(on/{d})':d={d}:s=1080x1920:fps=30",
]
...
def create_segment_clip(image_path, audio_path, sentence, seg_duration,
    ...
    dt_filters.append(
        f"drawbox=x=50:y=1850:w=980:h=8:color=0x333333@0.8:t=fill"
    )
    dt_filters.append(
        f"drawbox=x=50:y=1850:w={bar_filled}:h=8:color=0xFFD700@0.9:t=fill"
    )

    # Fact counter
    dt_filters.append(
        f"drawtext=fontfile='{font_path}':"
        f"text='FACT {seg_index+1} of {total_segs}':"
        f"fontsize=32:fontcolor=0xFFD700:"
