...
HUGGINGFACE_API = "https://router.huggingface.co/hf-inference/models/black-forest-labs/FLUX.1-schnell/v1/images/generations"
...
def fetch_huggingface_image(prompt, output_path):
    """AI-generated image via HuggingFace FLUX.1-schnell router API."""
    try:
        ...
        resp = requests.post(HUGGINGFACE_API, headers=headers, json=payload, timeout=120)
        resp.raise_for_status()
        data = resp.json()
        # Response is either b64_json or a URL
        import base64
        b64 = data.get("data", [{}])[0].get("b64_json")
        if b64:
            with open(output_path, "wb") as f:
                f.write(base64.b64decode(b64))
            logger.info(f"HuggingFace image saved (b64)")
            return output_path
        # Try URL format
        ...
        if img_url:
            ...
            with open(output_path, "wb") as f:
                f.write(r.content)
            logger.info(f"HuggingFace image saved (url, {len(r.content)//1024}KB)")
            return output_path
        return None
    except Exception as e:
...
def generate_backgrounds(topic, out_dir, count=4):
    """Generate background images: Pollinations AI first, Pexels fallback."""
    os.makedirs(out_dir, exist_ok=True)
    pexels_key = os.environ.get("PEXELS_API_KEY", "")
    title = topic.get("title", "science facts")
    subject = topic.get("pexels_search", title)

    ai_prompts = [
        f"ultra realistic cinematic {subject}, dramatic lighting, 8k photography, vertical portrait",
        f"close up macro shot of {subject}, stunning detail, dramatic shadows, cinematic color grading",
        f"breathtaking {subject} in natural habitat, National Geographic style, golden hour lighting",
        f"epic wide angle {subject}, vibrant colors, cinematic photography, dark moody atmosphere",
    ]
    ...
    for i in range(count):
        ...
        result = fetch_huggingface_image(ai_prompts[i % len(ai_prompts)], img_path)

        # Pexels as fallback
        if not result and pexels_key:
            result = fetch_pexels_photo(subject, pexels_key, img_path)
