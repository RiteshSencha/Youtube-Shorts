...
from video_assembler import generate_ai_image, assemble_video, get_audio_duration
...
def run_pipeline(config, test_mode=False):
    ...
    try:
        ...
        # 4. Generate AI background image
        logger.info("=== STEP 4: Generating AI background image ===")
        ai_image_path = f"output/background_{timestamp}.jpg"
        pexels_queries = config["video"]["pexels_queries"]
        image_prompt = topic.get("pexels_search", random.choice(pexels_queries))
        generate_ai_image(image_prompt, ai_image_path, seed=random.randint(1, 9999))
        metadata["steps"]["background"] = ai_image_path

        # 5. Assemble video with kinetic typography
        logger.info("=== STEP 5: Assembling video with kinetic text ===")
        final_path = f"output/final_{timestamp}.mp4"
        assemble_video(ai_image_path, audio_path, script["narration"], final_path)
